...
public class Boss1Manager : CharacterManager, IDataPersistence
{
    ...
    public override void TakeDamage(float damage, float poiseDamage)
    {
        ...
        if (currentHealth < 0)
        {
            ...
            for (int i = 0; i < amountOfItemsDropped; i++)
            {
                GameObject newItem = Instantiate(pickUpItemObj, transform.position, this.transform.rotation);
                ItemSlotManager newItemSlot = newItem.GetComponent<ItemSlotManager>();
                newItemSlot.itemFileLocation = droppedItemPossibility[Random.Range(0, droppedItemPossibility.Length -1)];
                newItemSlot.level = Random.Range(droppedItemQuantityMin, droppedItemQuantityMax);
                Rigidbody newItemRigidBody = newItem.GetComponent<Rigidbody>();
                Vector3 velocityOfItem = this.transform.forward;
                velocityOfItem.y = 4;
                velocityOfItem = Quaternion.AngleAxis(Random.Range(-45f, 45f), Vector3.up) * velocityOfItem;
                newItemRigidBody.linearVelocity = velocityOfItem;
            }
